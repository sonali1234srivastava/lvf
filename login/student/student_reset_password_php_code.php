...
$check = 1;
   if (empty($_POST["student_reset_password"]))
    ...
    else
     {
      ...
      if (strlen($student_reset_password)<6)
       {
        $check = 0;
   ...
   if (empty($_POST["student_confirm_reset_password"]))
    ...
    else
     {
      ...
      if (strlen($student_confirm_reset_password)<6)
       {
        $check = 0;
        echo '<div class="alert alert-danger">Password must be of minimum 6 Characters !</div>';
       }
			  ...
			  $query = "SELECT * FROM student_detail WHERE email = ? AND token = ?";  
                ...
                if($count > 0)  {
                        ...
                        if (mysqli_stmt_execute($query_prepare_statement)){
							...
							echo '<div class="alert alert-success">Password Reset Successfully, <a href = "http://localhost/leave_form/login/student/index.php">Click Here</a> to LOGIN.</div>';
		                }
		                else{
