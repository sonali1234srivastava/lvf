...
$columns = array('leave_id','student_name','room_number','roll_number', 'student_number', 'course_branch', 'semester', 'hostel_name', 'leave_period', 'day_from', 'day_to', 'reason', 'visiting_person_name', 'relation', 'address_contact_number', 'applicant_mobile_number', 'residence_address_number','student_signature', 'nowdate', 'status' , 'accept');
...
//$sub_array[] = '<div id="'.$row["leave_id"].'" data-column="warden_remark">' . $row["warden_remark"] . '</div>';
 ...
 $sub_array[] = '<div id="'.$row["hod_task_id"].'" data-column="hod_image_path"><a href = "'.$row["hod_image_path"].'" class="btn btn-success btn-xs" target="_blank">Click Here</a></div>';
   ...
   $sub_array[] = '<div id="'.$row["hod_task_id"].'" data-column="hod_image_path">Not Available</div>';   data-toggle="modal" data-target="#myModal"
     ...
     if($row["accept"] == 1)
   {
    $sub_array[] = '<input type="button" name="grant" class="btn btn-success btn-sm grant" id="'.$row["leave_id"].'" data-backdrop="static" disabled value="Granted">';
    $sub_array[] = '<input type="button" name="refuse" class="btn btn-danger btn-sm refuse" id="'.$row["leave_id"].'" data-backdrop="static" disabled value="Refuse">';
   } 
   else if($row["accept"] == 2){
    $sub_array[] = '<input type="button" name="grant" class="btn btn-warning btn-sm grant" id="'.$row["leave_id"].'" data-backdrop="static" disabled value="Grant">';
    $sub_array[] = '<input type="button" name="refuse" class="btn btn-danger btn-sm refuse" id="'.$row["leave_id"].'" data-backdrop="static" disabled value="Refused">';
   }
   else{
    $sub_array[] = '<input type="button" name="grant" class="btn btn-warning btn-sm grant" id="'.$row["leave_id"].'" data-backdrop="static" value="Grant">';
    $sub_array[] = '<input type="button" name="refuse" class="btn btn-danger btn-sm refuse" id="'.$row["leave_id"].'" data-backdrop="static" value="Refuse">';
   } 
