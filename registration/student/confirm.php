...
include '../../validate_input.php';
  ...
  if(!isset($_GET["email"]) || !isset($_GET["token"]))
  {
      redirect();
	   ...
	   $query = "SELECT id FROM student_detail WHERE email = ? AND token = ? AND is_email_confirmed = ?";
      ...
      if(mysqli_stmt_execute($query_prepare_statement))
    {
      ...
      if($query_num_rows > 0)
      { 
        $query = "UPDATE student_detail SET is_email_confirmed = '1' , token = '' WHERE email = '$email' ";
